...
* @note 限定小车xy方向上合加速度值小于MAX_ACC,并且设定小车x,y,z方向上移动速度分别为speed_x, speed_y, speed_pos_rot
* @note 单位为国际单位，max_rot=256/40=6.4rad/s	
* @author Xuanting Liu
*******************************************************************************/
void do_acc_handle_move(int speed_x,int speed_y,int speed_pos_rot, int PID_type)
{
	...
	switch(PID_type)
	{
		case SPEED_PID:
		{
			...
			do_move(speed_x,speed_y,speed_pos_rot, SPEED_PID);
		...
		case POSITION_PID:
		{
			last_speed_x = 0;
			last_speed_y = 0;
			do_move(0, 0, speed_pos_rot, POSITION_PID);
...
* @author Xuanting Liu
*******************************************************************************/
void do_move( int speed_x, int speed_y, int speed_pos_rot, int PID_type)
{
	int i = 0;
