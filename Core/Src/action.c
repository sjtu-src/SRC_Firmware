...
volatile int acc_x=0, acc_y=0, acc_z=0;
extern v_que x_que, y_que, z_que;
extern vu64 gap_counter;

extern int v_x, v_y, v_z;

vu8 is_action_shoot = 0;

extern unsigned char identify_success;

int shoot_power_curve_a = SHOOT_POWER_CURVE_A;
int shoot_power_curve_b = SHOOT_POWER_CURVE_B;
float dribbler_power_amp = DRIBBLER_POWER_AMP;
...
int do_move( int speed_x, int speed_y, int speed_rot )
{
	...
	double arrfront[] = {0,0,0,0};
	double arrback[] = {0,0,0,0};
	double arrright[] = {0,0,0,0};
	double arrleft[] = {0,0,0,0};
	

		
	//g_robot.wheels[0].speed = (float)(0.7610 * speed_x + 0.6953 * speed_y + 8.2 * vz);
	//g_robot.wheels[1].speed = (float)(-1.0463 * speed_x + 0.8576 * speed_y + 8.2 * vz);
	//g_robot.wheels[2].speed = (float)(-0.5841 * speed_x - 0.8927 * speed_y + 8.2 * vz);
	//g_robot.wheels[3].speed = (float)(0.9760 * speed_x - 1.0327 * speed_y + 8.2 * vz);
	
	/* 各轮子线速度(m/s)设定值 */
	for( i = 0; i < CHANNEL_NUM; i++ )
	{
		/* trasnform wheel angle */
		g_robot.wheels[i].speed = ( g_robot.sin_angle[ i ] * vx + g_robot.cos_angle[ i ] * vy + vz * g_robot.cos_mod_angle[i]);
	
		temp_wheelSpeed = g_robot.wheels[i].speed;
			    
		if (g_robot.wheels[i].speed > 0)
		{
			if(vx>0)
				temp_wheelSpeed -= arrfront[i] * fabs(vx);
			else temp_wheelSpeed -= arrback[i] * fabs(vx);
			if(vy>0)	
				temp_wheelSpeed -= arrright[i] * fabs(vy);
			else temp_wheelSpeed -= arrleft[i] * fabs(vy);
		}
		else if (g_robot.wheels[i].speed < 0)
		{
			if(vx>0)
				temp_wheelSpeed += arrfront[i] * fabs(vx);
			else temp_wheelSpeed += arrback[i] * fabs(vx);
			...
			if (temp_wheelSpeed * g_robot.wheels[i].speed > 0) // 不能反向
				g_robot.wheels[i].speed = temp_wheelSpeed;
		
		
		
			
		//g_robot.wheels[i].speed = g_robot.wheels[i].speed / 100;
...
{
	while((abs(g_robot.wheels[0].cur_speed) > STOP_THRESHOLD) || (abs(g_robot.wheels[1].cur_speed) > STOP_THRESHOLD) ||
		  (abs(g_robot.wheels[2].cur_speed) > STOP_THRESHOLD) || (abs(g_robot.wheels[3].cur_speed) > STOP_THRESHOLD))
	{
		set_motor_dir(1, 0, 0, 1); //电机正转
		set_motor_pwm(6720, 6720, 6720, 6720); //电机转动
	}
	last_speed_x = 0;
	last_speed_y = 0;
	last_speed_rot = 0;
...
{   
    ...
    if((( (g_robot.mode == NORMAL_MODE) || (g_robot.mode == CRAY_MODE) ) && (identify_success == 1)) || g_robot.mode == PID_TUNE_MODE || g_robot.mode == SHOOT_POWER_CURVE_TUNE_MODE) //认证成功才能运行
		{
		...
		acc_x = v_output(&x_que, packet->speed_x);
		acc_y = v_output(&y_que, packet->speed_y);
		acc_z = v_output(&z_que, packet->speed_rot);
		v_x = packet->speed_x;
		v_y = packet->speed_y;
		v_z = packet->speed_rot;
    }
...
{
	int last_idx;
	k->t[k->t_idx] = gap_counter;
	gap_counter = 0;
	last_idx = k->t_idx;
	k->t_idx = (k->t_idx+1)%2;
	k->speed[k->idx] = v;
	
	k->acc = ((k->speed[k->idx]-k->speed[k->idx_t])/k->t[last_idx]+(k->speed[k->idx_t]-k->speed[k->idx_prim])/k->t[k->t_idx])/2;
...
{
	int error = k->e_error + k->p_error;
	k->x_k = k->x_k + acc*5;
	k->kar = error / (error+k->m_error);
	k->x_k = k->x_k + k->kar*(k->m_k - k->x_k);
	k->e_error = (1-k->kar)*error;
}
